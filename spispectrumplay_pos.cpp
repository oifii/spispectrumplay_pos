...
BOOL PlayFile(const char* filename)
{
	...
	global_length_byte=BASS_ChannelGetLength(chan,BASS_POS_BYTE);
	if(global_fSecondsPlay<=0)
	{
		//QWORD length_byte=BASS_ChannelGetLength(chan,BASS_POS_BYTE);
		global_fSecondsPlay=BASS_ChannelBytes2Seconds(chan,global_length_byte);
	}
	...
	switch (m) {
		...
		case WM_LBUTTONDOWN:
			...
			else
			{
				...
				*/
				QWORD position_byte = (QWORD)(((xmousepos+0.0f)/(SPECWIDTH+0.0f))*(global_length_byte));
				BASS_ChannelSetPosition(chan, position_byte, BASS_POS_BYTE);
			}
			return 0;
		case WM_LBUTTONUP:
			if(w==MK_SHIFT)
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (!RegisterClass(&wc) || !CreateWindow("BASS-Spectrum",
			...
			"spispectrumplay (click to change vol)",
